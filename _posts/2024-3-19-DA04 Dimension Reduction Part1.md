...
$1^\circ$ 构建 simple regression model ，如 $y = \beta_0 + \beta_1x_1$的形式，总共会有 $C_5^1$ 种 simple regression 模型；

$2^\circ$ 构建复杂的模型，分别选 2， 3， 4， 5 个 regressors ，可以分别得到 $C_5^2$ , $C_5^3$, $C_5^4$ , $C_5^5$ 种模型。
...
$1^\circ$ 先对 variables 进行标准化，然后做回归。这样统一了量纲 ( same scale )，$\beta$ 的值就变得**有意义了**。可以说明哪些 regressor 很重要。
...
$2^\circ$ 当使用 stepwise 方法移除变量时，利用 adjusted R square 进行筛选。
